...
## data.table -- this will modify in place
flight_dt[, month_day := paste(month, day, sep = "-")][1:3]
...
```{r}
## data.table -- this will modify in place
flight_dt[, mean_dep_delay := mean(dep_delay), by = .(carrier, origin, dest)][1:2]
```
...
* can make certain functions even faster (e.g., lookups in key columns) and allows for merging without specifying merging columns
