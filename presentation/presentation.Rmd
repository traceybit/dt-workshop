...
--
    * If you're used to using `dplyr` or base r while working with large data sets in R, `data.table` will feel *fast*.
...
NOTE TO MEAS: I'm having a hard time creating something that show dt being much faster
...
## keys NOTE TO MEAS: not sure if i'm really getting this
keys are like row names. setting keys...

* reorders the rows of a data.table by the key columns in increasing order

* marks those columns as key columns by setting attribute called `sorted`

* can make certain functions even faster
...
## it's soooo fast!!! what shoudl we add?
